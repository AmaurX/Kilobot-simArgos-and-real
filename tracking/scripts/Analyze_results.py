...
def main():
        ...
        for element in files:
            if element.endswith('time_results.tsv'):
                complete_time_dict, total_num_robot = first_discovery(
                    directory, element, number_of_robots, total_num_robot, complete_time_dict)
            else:
                continue
        ...
        for element in files:
            if element.endswith('position.tsv'):
                ...
                (n_sec_distance_list, total_number_of_robots) = n_sec_dist(directory, element,
                                                                           n_sec_distance_list, n, total_number_of_robots, number_of_robots)
    ...
    plt.title("Robot histogram of speed. Arena diameter: " + arena_size + "m " +
              str(number_of_robots) + " kilobots per run, " + str(w_displacement_run_count) + " runs")
    plt.legend()
    plt.savefig(folder + "/Speed_"+arena_size + "m*" + arena_size +
                "m_" + str(number_of_robots) + "kilobots_" + str(w_displacement_run_count) + " runs.png", bbox_inches='tight', dpi=200, orientation="landscape")
    # plt.show(block=False)
    plt.close()


def n_sec_dist(directory, element, n_sec_distance_list, n, total_number_of_robots, number_of_robots):
    ...
    for row in tsvin:
        if(row[0] != "Robot id"):
            expe_length = len(row) - window_size
            for i in range(1+2*n, len(row) - 1):
                distance = 0.0
                for j in range(0, 2*n, 2):

                    [xi, yi] = row[i - j - 2].split(",")
