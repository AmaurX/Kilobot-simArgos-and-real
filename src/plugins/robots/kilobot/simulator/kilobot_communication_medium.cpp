...
CKilobotCommunicationMedium::CKilobotCommunicationMedium() : m_pcKilobotIndex(NULL),
...
{
    return reinterpret_cast<size_t>(c_pair.first) ^
                 ...
                 ++it2)
            {
                    ...
                    cOtherKilobot.GetTxStatus() == CKilobotCommunicationEntity::TX_ATTEMPT)
                {
                    ...
                    if (itPair == mapPairsAlreadyChecked.end() || /* Pair does not exist */
                        ...
                        itPair->second.second != cTestKey.second)
                    { /* Pair exists, but second Kilobot involved is different */
